...
def verif_capacite(population, poids, capacite):
    for i in range(population.shape[0]):
        if (np.sum(population[i] * poids) > capacite):
            # recuperer l'indice de chaque 1 dans la solution
            ones = [one for one in range(population.shape[1]) if population[i][one] == 1]
            while (np.sum(population[i] * poids) > capacite):
...
verif_capacite(population_initiale, poids, capacite_max)
...
def optimize(poids, valeur, population, pop_size, nbr_generations, capacite):
    ...
    for _ in range(nbr_generations):
        ...
        verif_capacite(enfants, poids, capacite)

        mutants = mutation(enfants)

        verif_capacite(mutants, poids, capacite)
